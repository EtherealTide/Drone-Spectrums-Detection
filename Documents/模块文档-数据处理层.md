...
数据处理层是连接通信层和算法层/UI层的中间模块，负责接收原始FFT数据、进行批量信号处理、维护正方形瀑布图数据结构并转换为RGB图像，提供线程安全的数据访问接口。
...
- 从队列批量接收原始FFT数据
- 进行全局归一化处理
- 实时维护正方形瀑布图buffer（1024×1024）
- 双线程异步转换RGB图像供算法层使用
- 提供线程安全的数据访问接口
- 统计处理帧数和数据范围

**核心优化**：
- **批量处理**：一次性获取队列中所有数据，减少锁竞争
- **全局归一化**：对一批数据统一归一化，避免亮度跳变
- **双线程分离**：数据处理和图像转换分离，提高效率
- **正方形设计**：1024×1024瀑布图适配YOLO检测需求
...
self.image_thread: Thread               # RGB图像转换线程
self.data_lock: threading.Lock          # 保护buffer数据
self.image_lock: threading.Lock         # 保护RGB图像

# 数据队列
self.fft_data_queue: queue.Queue        # 从通信层接收FFT数据

# 频谱数据
self.latest_spectrum: np.ndarray        # 最新频谱（归一化到[0,1]）

# 瀑布图参数（正方形）
self.fft_length: int                    # FFT长度（默认1024）
self.waterfall_height: int              # 瀑布图高度 = fft_length
self.waterfall_width: int               # 瀑布图宽度 = fft_length

# 瀑布图数据结构
self.waterfall_buffer: deque            # 瀑布图历史缓冲（归一化数据[0,1]）
self.waterfall_image: np.ndarray        # RGB图像（uint8，shape=(height, width, 3)）

# 图像转换参数
self.transfer_time: float = 0.01        # 图像更新间隔（10ms）
self.image_needs_update: bool           # 图像更新标志
self.colormap: np.ndarray               # Jet色图映射表（256×3）

# 信号处理参数（可选）
self.enable_averaging: bool = False     # 是否启用移动平均
self.averaging_count: int = 10          # 移动平均窗口大小
self.history_buffer: deque              # 历史数据缓冲

# 统计信息
self.processed_frame_count: int = 0     # 已处理帧数
self.max_value: float = 0.0             # 当前最大归一化值
self.min_value: float = 0.0             # 当前最小归一化值
self.batch_size: int = 0                # 上次批量处理的帧数
```
